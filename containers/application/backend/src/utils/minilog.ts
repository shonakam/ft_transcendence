...
class MiniLog {
  ...
  private purgeOldLogs() {
    const files = fs.readdirSync(this.logDir)
      ...
      .map((f: any) => ({ name: f, time: fs.statSync(path.join(this.logDir, f)).mtime.getTime() }))
  ...
  public i(tag: string, msg: string) { this.log(tag, msg, 'INFO') }
  public e(tag: string, msg: string) { this.log(tag, msg, 'ERROR') }
  public d(tag: string, msg: string) { this.log(tag, msg, 'DEBUG') }
  public w(tag: string, msg: string) { this.log(tag, msg, 'WARN') }
}
