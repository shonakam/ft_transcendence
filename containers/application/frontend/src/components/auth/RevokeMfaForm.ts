...
export class RevokeMfaForm implements Component {
  ...
  constructor() {
    ...
    this.root.className = 'flex flex-col space-y-4 p-6 bg-rose-500/5 border border-rose-500/20 rounded-xl'
    ...
    description.textContent = 'セキュリティ保護のため、現在の認証コードを入力して解除を確定してください。'

    this.codeInput = document.createElement('input')
    this.codeInput.type = 'text'
    this.codeInput.maxLength = 6
    this.codeInput.placeholder = '000000'
    this.codeInput.className = 'w-full px-4 py-3 rounded-lg bg-white/5 text-white text-center text-2xl font-mono tracking-widest border border-white/10 focus:border-rose-500 outline-none transition-colors'

    this.submitButton = document.createElement('button')
    this.submitButton.type = 'submit'
    this.submitButton.className = 'w-full py-3 rounded-lg bg-rose-600 text-white font-bold hover:bg-rose-500 transition-all active:scale-[0.98]'
  ...
  private initEvents() {
    this.root.addEventListener('submit', async (e) => {
      ...
      if (!confirm('本当に2段階認証を解除しますか？セキュリティレベルが低下します。')) return

      this.setLoading(true)

      // バックエンドの DELETE /auth/revoke-mfa/totp を叩く
      // ボディに code を含める（Fastify側のスキーマに合わせて調整）
      const [_, err] = await to(api.delete('auth/revoke-mfa/totp', {
