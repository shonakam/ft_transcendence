...
export class UserMenu implements Component {
  ...
  constructor() {
    ...
    this.el.className = 'fixed z-50 bg-slate-800 border border-white/10 rounded-lg shadow-2xl py-2 w-48 hidden animate-in fade-in zoom-in duration-200';
  ...
  private render() {
    ...
    label.className = 'px-4 py-2 text-xs text-slate-400 border-b border-white/5 mb-1';
    ...
    const blockBtn = this.createOption('🚫 Block User', async () => {
      if (!this.currentUserId) return;
      if (confirm(`Are you sure you want to block ${this.currentUsername}?`)) {
        try {
          await chatService.blockUser(this.currentUserId);
          window.dispatchEvent(new CustomEvent('user-blocked', { detail: this.currentUserId }));
        } catch (error) {
  ...
  private createOption(text: string, onClick: () => void, className: string = 'text-white hover:bg-white/5'): HTMLButtonElement {
