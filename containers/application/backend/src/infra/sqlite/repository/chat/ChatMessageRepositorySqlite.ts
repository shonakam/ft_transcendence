  ...
  async save(message: ChatMessage): Promise<void> {
    await this.db.run(
      `INSERT INTO chat_messages (id, room_id, sender_id, content, message_type, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [message.id, message.roomId, message.senderId, message.content, message.messageType, message.createdAt]
    );
  }

  async findByRoomId(roomId: string, limit: number, offset: number): Promise<ChatMessage[]> {
    const rows = await this.db.all(
      `SELECT * FROM chat_messages WHERE room_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [roomId, limit, offset]
    );
    return rows.map((row) => this.scan(row)).reverse();
  }
}
